...
cloudflare/
├── defaults/
│   └── main.yml           # Generic defaults shared across all inventories
├── vars/
...
└── documents/
    ...
    └── PLATFORM.md        # Platform configuration template
```
...
The role loads configuration from multiple locations:

**Role-Level (Generic Across All Inventories):**
```
roles/linux/cloudflare/
├── defaults/
│   └── main.yml                            # Generic defaults (domains, DNS standards)
└── vars/
...
```
inventories/{IOM|LON|MIP|NJP|ONP|PAP}/
├── hosts                                    # Inventory hosts file
└── group_vars/
    └── all/
        ├── credentials.yml                  # Cloudflare API credentials (Vault encrypted)
        ├── cloudflare.yml                   # Environment-specific settings
        └── platforms/                       # Platform configs (5-15 lines each)
            ├── live/                        # Live/Production platforms (L008-L025)
            │   ├── L008.yml
            │   ├── L012.yml
            │   └── ...
            ├── preprod/                     # Pre-production platforms (P000-P019)
            │   ├── P000.yml
            │   ├── P009.yml
            │   └── ...
...
### Environment Settings (all/cloudflare.yml)

**Inventory-specific** configuration for environment settings:

**IOM Inventory** (`inventories/IOM/group_vars/all/cloudflare.yml`):
```yaml
---
default_location: "IOM"
default_region: "Isle of Man"

# IOM manages: Preprod, Staging, Trunk, Global on iforium.com
preprod_environment_settings:
  ssl_mode: "full_strict"
  always_use_https: true
  dns_proxied: true

staging_environment_settings:
  ssl_mode: "full_strict"
  always_use_https: true
  dns_proxied: true

trunk_environment_settings:
  ssl_mode: "off"
  always_use_https: false
  dns_proxied: true

global_environment_settings:
  ssl_mode: "full_strict"
  always_use_https: true
  dns_proxied: true
```

**LON Inventory** (`inventories/LON/group_vars/all/cloudflare.yml`):
```yaml
---
default_location: "LON"
default_region: "London"

# LON manages: Live platforms ONLY on game-flex.eu
live_environment_settings:
  ssl_mode: "full_strict"
  always_use_https: true
  dns_proxied: true
```

### Role Defaults (roles/linux/cloudflare/defaults/main.yml)

**Generic configuration** shared across all inventories:
